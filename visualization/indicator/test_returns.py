...
def test_returns(symbols, start_date: datetime, end_date: datetime,ind_return,ax):
    ...
    df_main[[ind_return.get_column_name(symbol) for symbol in symbols]].plot(ax=ax)
...
test_returns(['SPX','AAPL','MSFT'], start_date, end_date, CummulativeDailyReturn(), axes[1])
